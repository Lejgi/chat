...
local function logToDiscord(title, message, color)
    local webhook = 'https://discord.com/api/webhooks/1369347955718492210/jNwdtmsDMEnr_Afv0Yt6V8ROiXBl9Iv_uhke29XHeDP_zmLYOlw8VRRTxjRtH8lxRFsb' 
    local embed = {
        {
            title = title,
            description = message,
...
RegisterServerEvent("pivovar:harvestPlant")
AddEventHandler("pivovar:harvestPlant", function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)

    if xPlayer then
        xPlayer.addInventoryItem("lemon", math.random(1,5))
        TriggerClientEvent("ox_lib:notify", src, {
            type = "success",
            description = "Sebral jsi rostlinu."
        })
    end
end)

RegisterNetEvent('pivovar:collectWater', function(item, count)
...
ESX.RegisterServerCallback('pivovar:checkJob', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer and xPlayer.job.name == 'pivovar' then
        cb(true)
    else
        cb(false)
    end
...
RegisterServerEvent('pivovar:craft')
AddEventHandler('pivovar:craft', function(type)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    ...
    TriggerClientEvent('pivovar:playCraftAnim', src, type)
end)

RegisterServerEvent('pivovar:finishCraft')
AddEventHandler('pivovar:finishCraft', function(type)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
...
RegisterNetEvent("pivovar:sellItem")
AddEventHandler("pivovar:sellItem", function(item, amount)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    local count = exports.ox_inventory:Search(src, 'count', item)

    if not xPlayer or xPlayer.job.name ~= "pivovar" then return end
...
RegisterNetEvent("pivovar:buyItem", function(itemName, amount, price)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
...
lib.callback.register('pivovar:getItemCount', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    local count = exports.ox_inventory:Search(source, 'count', 'dirty_bottle2')
    return count
end)

lib.callback.register('pivovar:getItemCount2', function()
  local xPlayer = ESX.GetPlayerFromId(source)
  local count = exports.ox_inventory:Search(source, 'count', 'sponge')
  return count
end)

RegisterNetEvent('pivovar:washbottle')
AddEventHandler('pivovar:washbottle', function()
    ...
    if xPlayer.job.name == "pivovar" then
        if xPlayer ~= nil then
            if xPlayer.canCarryItem('bottle2', 5) then
                if count >= 5 then
                    exports.ox_inventory:RemoveItem(source, 'dirty_bottle2', 5)
                    exports.ox_inventory:AddItem(source, 'bottle2', 5)
                elseif count < 5 then
                    TriggerClientEvent('ox_lib:notify', source, {
                        title = "Bar",
...
lib.callback.register('pivovar:canCarry', function(source, item, count)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
