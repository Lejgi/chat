...
for categoryName, category in pairs(Config.CraftOptions) do
    for _, option in pairs(category) do
        if option.event == "lihovar:craft" and option.args then
            ...
            option.onSelect = function()
                TriggerServerEvent("lihovar:craft", option.args)
            end
        end
    end
end
...
local function registerCraftingTargets()
    ...
    for i, zone in pairs(Config.CraftingZones) do
        local id = "lihovar_craft_target_" .. i
...
RegisterNetEvent('lihovar:playCraftAnim', function(type)
    ...
    TriggerServerEvent('lihovar:finishCraft', type)
end)
