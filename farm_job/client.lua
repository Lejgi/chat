...
local function harvest(data)
    if not hasJob() then
        lib.notify({type = 'error', description = _L('not_farmer')})
        return
    end
    if lib.progressBar({duration = Config.Progress.Harvest, label = _L(data.label)}) then
...
local function processItem(data)
    ...
    if lib.progressBar({duration = Config.Progress.Process, label = _L(data.label)}) then
...
local function cookItem(data)
    ...
    if lib.progressBar({duration = Config.Progress.Cook, label = _L(data.label)}) then
...
local function sellItem(data)
    ...
    if lib.progressBar({duration = Config.Progress.Sell, label = _L(data.label)}) then
